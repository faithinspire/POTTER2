-- Migration: Enhanced Customer Features and Analytics (FIXED VERSION)
...
-- Create function to calculate daily payment schedules (FIXED)
...
-- Create function to update agent performance metrics (FIXED)
...
BEGIN
    ...
    IF metrics.total_amount_disbursed > 0 THEN
        v_collection_rate := (metrics.total_amount_collected / metrics.total_amount_disbursed) * 100;
    END IF;
...
-- Create function to update branch analytics (FIXED)
...
BEGIN
    ...
    IF analytics.total_disbursed > 0 THEN
        v_collection_rate := (analytics.total_collected / analytics.total_disbursed) * 100;
    END IF;
